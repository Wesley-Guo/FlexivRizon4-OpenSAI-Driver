...
void PeriodicTask(flexiv::Robot &robot, flexiv::Model &model, flexiv::Log &log,
  ...
  try {
    ...
    if (n_limited_joints > 0) {
      for (int i = 0; i < 7; ++i) {
        if (_pos_limit_flag[i] == MIN_SOFT) {
          // ramping kv damping proportional to violation difference up to max
          // damping + command torques (goes from tau -> tau + max damping)
          // double kv = kv_safety[i] * (robot_state.q[i] - soft_min_angles[i])
          ...
          // double kv = kv_safety[i] * (robot_state.q[i] - soft_max_angles[i])
          // / (hard_max_angles[i] - soft_max_angles[i]);
          double alpha = getBlendingCoeff(robot_state.q[i], soft_max_angles[i],
                                          hard_max_angles[i]);
          _tau_limited(i) = _tau(i) - alpha * kv_safety[i] * robot_state.dq[i];

        } else if (_pos_limit_flag[i] == MIN_HARD) {
          // max damping + command torques blend with quadratic ramping (goes
          // from tau + max damping -> holding tau + max damping with blending
          // from tau to holding tau) double dist = (robot_state.q[i] -
          // hard_min_angles[i]) / (joint_position_min[i] - hard_min_angles[i]);
          ...
          // command torques blend with holding torques to soft joint velocity
          // (goes from tau -> tau hold with blending)
          ...
          // _torque_scaling_vector(i) = soft_min_joint_velocity_limits[i] /
          // robot_state.dq[i]; _tau(i) = 0;
          ...
          _tau_limited(i) = (1 - std::pow(alpha, 1)) * _tau(i);
          // _torque_scaling_vector(i) = (1 - std::pow(alpha, 4));
          // _torque_scaling_vector(i) = soft_max_joint_velocity_limits[i] /
          // robot_state.dq[i]; _tau(i) = 0;

        } else if (_vel_limit_flag[i] == MIN_HARD_VEL) {
          // full damping with soft joint velocity limits goal and a quadratic
          // ramp to max torque
          double alpha = getBlendingCoeff(robot_state.dq[i],
                                          hard_min_joint_velocity_limits[i],
                                          -joint_velocity_limits_default[i]);
          // _tau_vel_limited(i) = - kv_safety[i] * std::pow(alpha, 2) *
          // (robot_state.dq[i] - 0 * hard_min_joint_velocity_limits[i]);
          // _torque_scaling_vector(i) = soft_min_joint_velocity_limits[i] /
          // robot_state.dq[i]; _torque_scaling_vector(i) = 0; _tau_limited(i) =
          // - kv_safety[i] * std::pow(alpha, 4) * (robot_state.dq[i] - 0 *
          ...
          double alpha = getBlendingCoeff(robot_state.dq[i],
                                          ...
                                          joint_velocity_limits_default[i]);
          // _tau_vel_limited(i) = - kv_safety[i] * std::pow(alpha, 2) *
          // (robot_state.dq[i] - 0 * hard_max_joint_velocity_limits[i]);
          // _torque_scaling_vector(i) = 0;
          // _torque_scaling_vector(i) = soft_max_joint_velocity_limits[i] /
          // robot_state.dq[i]; _tau_limited(i) = - kv_safety[i] *
          // std::pow(alpha, 4) * (robot_state.dq[i] - 0 *
          // hard_max_joint_velocity_limits[i]);
          _tau_limited(i) =
              -std::pow(alpha, 4) * joint_torques_limits_default[i] * 1e-2;
          // _tau(i) = 0;
    ...
    //         safety_mode_flag = true;
    //         if (safety_controller_count == 200) {
    //             std::cout << "WARNING : End effector too low, engaging safety
    //             mode" << std::endl; std::cout << "position of monitoring
    //             point : " << pos_monitoring_point.transpose() << std::endl;
    //         }
    //     }
    //     // cylinder
    ...
    //         safety_mode_flag = true;
    //         if (safety_controller_count == 200) {
    //             std::cout << "WARNING : End effector too close to center of
    //             workspace, engaging safety mode" << std::endl; std::cout <<
    //             "position of monitoring point : " <<
...
int main(int argc, char **argv) {
  ...
  if (robot_sn == "Rizon4s_062232") {
    WRIST_FORCE_TORQUE_SENSED_KEY =
        "OpenSai::FlexivRizon::" + robot_id[robot_sn] + "::sensors::wrist_FT";
  }
  ...
  try {
    ...
    // =========================================================================================
    // IMPORTANT: must zero force/torque sensor offset for accurate force/torque
    // measurement
    ...
    // otherwise the result will be inaccurate and affect following operations
    log.Info("Zeroing force/torque sensors, make sure nothing is in contact "
             "with the robot");
